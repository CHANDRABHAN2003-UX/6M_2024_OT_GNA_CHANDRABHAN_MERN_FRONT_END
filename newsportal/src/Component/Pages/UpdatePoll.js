...
export default function UpdatePoll() {
    ...
    const id = params.id
    useEffect(() => {
        axios.post("http://localhost:5050/admin/single/poll", { _id: id }, { headers: { Authorization: sessionStorage.getItem("token") } })
            .then((res) => {
                console.log(res);
                console.log(res.data.data.pollStatus);
                setPollStatus(res.data.data?.pollStatus);
                setQuestion(res.data?.data?.pollQuestion)
                setTotalOption(res.data.data.totalOption)
